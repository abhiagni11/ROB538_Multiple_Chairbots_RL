...
# 			self._all_agents[elem] = (0, 0, randint(0, self._Nx), randint(0, self._Ny), -1)
	...
	def add_new_group_of_people(self):
		...
		while self._number_of_groups_present < self._max_number_of_groups:
			if ((self._step < (iter_step * 10)) and (self._step % iter_step == 0)):
				self._groups_of_people[self._number_of_groups_present] = (float(int(self._Nx/2)), 0, randint(2,2))
	...
	def visualize_restaurant(self):
		...
		# Plot entrance
		circle_entrance = plt.Circle((self._Nx/2+1, 1), .2, color='g')
		self.ax.add_artist(circle_entrance)
		self.ax.text(self._Nx/2 - 2, -2, 'ENTRANCE')
...
# model_path = '/home/risheek/Pytorch_workspace/dqn_1'

policy_net_agent_1 = DQN()
policy_net_agent_1.double()

# policy_net_agent_1.load_state_dict(torch.load(model_path))
...
optimizer_agent_1 = optim.RMSprop(policy_net_agent_1.parameters(),lr=1e-3)
